...
def train(model, device, train_ds, val_ds, train_opts, exp_dir=None):
    ...
    for i in range(train_opts['num_epochs']):
        model.train()
        tr_l, tr_a = fit(model, train_dl, criterion, device, optimizer)
        tr_loss.append(tr_l)
        tr_acc.append(tr_a)

        # validation
        model.eval()
        val_l, val_a = fit(model, val_dl, criterion, device)
...
def fit(model, data_dl, criterion, device, optimizer=None):
    """
